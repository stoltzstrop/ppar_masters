...
/*
 *  very rough test code for opencl using local memory
 *  the main differences can be seen in the kernels.cl file 
 *  the Z grid side must be skipped in workitem settings as well
 *
 */
...
// debugging to check for differences in timings between say, 1st and 4000th kernel
void writeEventsToFile(int numEvents, vector<cl::Event> events, char* outfile)
{
    char wholeFile[1024];
    ...
    {
        fprintf(file, "%f\n", CLUtils::GetEventTiming(events[i]));
    }
...
int main(){
	...
	// Set up grid and blocks
        ...
        int platformIdx = platforms.size()-1;
        ...
        if(program2.build(devices) != CL_SUCCESS)
        {
	...
	{
            err = queue2.enqueueNDRangeKernel(stencil_kernel2, cl::NullRange, cl::NDRange(Nx,Ny,1), 
                                                        cl::NDRange(BLOCK_X,BLOCK_Y,1), NULL, &kernel1Events[n]);
            err_check(err);
            ins = 0.0;
	    if(n<alength)
            {
    ...
    writeEventsToFile(numberSamples, kernel1Events, "kernel1events.clog");


    printToString;
    printOutputs;
    writeTimingsToFile; 
    printf("Total::: %f\n",dataCopyTotal + programBuildTotal + kernel1Time + kernel2Time);
